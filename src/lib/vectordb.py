...
from lib.context import embedding_model_context, reranker_context
from src.lib.schema import Chunk, Document
...
class VectorDB:
    def __init__(
        ...
        self.db_path = f"./data/{name}/vectordb.pkl"
    ...
    def insert(self, paths: list[str]) -> None:
        ...
        assert any(os.path.exists(path) for path in paths), (
    ...
    def _insert(
        ...
        assert any(isinstance(doc, Document) for doc in docs), (
    ...
    def load_db(self) -> None:
        ...
        if not os.path.exists(self.db_path):
            raise ValueError("Database not found. Use load_data first.")
        with open(self.db_path, "rb") as file:
            data = pickle.load(file)
        self.documents = data["documents"]
        self.chunks = data["chunks"]
        self.dense_embeddings = data["dense_embeddings"]
        self.sparse_embeddings = data["sparse_embeddings"]
